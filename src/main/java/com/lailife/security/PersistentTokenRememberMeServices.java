...
public class PersistentTokenRememberMeServices extends
    ...
    protected void onLoginSuccess(HttpServletRequest request, HttpServletResponse response, Authentication
        ...
        String loginOrPhone = successfulAuthentication.getName();

        log.debug("Creating new persistent login for user {}", loginOrPhone);
        PersistentToken token = userRepository.findOneByLoginOrPhone(loginOrPhone).map(u -> {
    ...
    protected void onLoginFail(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            response.setStatus(HttpStatus.OK.value());
            response.getWriter().print(Message.fromResponseStatus(ResponseStatus.FAILED).toJson());
        } catch (IOException e) {
            response.setStatus(HttpStatus.FORBIDDEN.value());
            e.printStackTrace();
        }
